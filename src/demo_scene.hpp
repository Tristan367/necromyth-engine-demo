...
[[nodiscard]] inline auto asset_path(std::string_view relative) -> std::string {
  return std::string(APP_ASSETS_DIR) + std::string(relative);
}

[[nodiscard]] inline auto engine_asset_path(std::string_view relative) -> std::string {
...
[[nodiscard]] inline auto add_cached_texture(
    engine::Scene &scene,
...
[[nodiscard]] inline auto add_gltf_mesh(engine::Scene &scene, const engine::LoadedGltfPrimitive &primitive) -> std::uint32_t {
  return scene.add_mesh({.vertices = primitive.mesh.vertices, .indices = primitive.mesh.indices});
}

[[nodiscard]] inline auto texture_for_gltf_material(
    engine::Scene &scene,
    std::unordered_map<std::string, std::uint32_t> &cache,
...
inline void add_gltf_model_instances(
    engine::Scene &scene,
...
inline void populate_demo_scene(engine::Scene &scene) {
  std::unordered_map<std::string, std::uint32_t> texture_cache;
  ...
  scene.directional_light().direction_toward_light = glm::normalize(glm::vec3(0.35F, 0.85F, 0.38F));
...
[[nodiscard]] inline auto create_demo_scene() -> engine::Scene {
  engine::Scene scene;
  populate_demo_scene(scene);
  return scene;
}

inline void update_demo_scene(engine::Scene &scene) {
  static const auto start_time = std::chrono::high_resolution_clock::now();
  const auto current_time = std::chrono::high_resolution_clock::now();
  const float time = std::chrono::duration<float>(current_time - start_time).count();

  if (scene.instances().size() >= 11)
    scene.instance(10).model = glm::rotate(glm::mat4(1.0F), time * glm::radians(90.0F), glm::vec3(0.0F, 1.0F, 0.0F)) *
